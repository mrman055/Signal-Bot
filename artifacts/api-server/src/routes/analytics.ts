...
router.get("/analytics/top-signals", async (req, res) => {
...
router.get("/analytics/market-overview", async (_req, res) => {
  const signals = await getAllSignals();
  const markets = ["crypto", "forex", "commodity"] as const;

  const overview: Record<string, {
    total: number; buy: number; sell: number; neutral: number;
    avgStrength: number; topSignal: string | null;
  }> = {};
  ...
  for (const market of markets) {
    ...
    const avgStrength =
      mkt.length > 0
        ...
        : 0;
    const top = mkt.filter((s) => s.direction !== "NEUTRAL").sort((a, b) => b.strength - a.strength)[0];
    overview[market] = { total: mkt.length, buy, sell, neutral, avgStrength, topSignal: top ? top.symbol : null };
  }
