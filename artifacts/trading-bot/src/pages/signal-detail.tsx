...
import { useGetSignalBySymbol, useRefreshSignal, useFetchCandles, SignalDirection } from "@workspace/api-client-react";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { ArrowLeft, ArrowUpRight, ArrowDownRight, Minus, RefreshCw, AlertTriangle, Target, LineChart, Activity } from "lucide-react";
import { Link } from "wouter";
import { useQueryClient } from "@tanstack/react-query";
import { getGetSignalBySymbolQueryKey } from "@workspace/api-client-react";
...
export default function SignalDetail() {
  ...
  const { data: candles } = useFetchCandles(symbolStr, {
    query: {
      enabled: !!symbolStr
    }
  });

  const refreshMutation = useRefreshSignal({
    mutation: {
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: getGetSignalBySymbolQueryKey(symbolStr) });
        toast({ title: "Signal Refreshed", description: `Analysis updated for ${symbolStr}` });
      }
    }
  });

  if (isLoading) {
    return <div className="p-8 flex justify-center items-center h-64"><RefreshCw className="h-8 w-8 animate-spin text-primary" /></div>;
  }
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground uppercase tracking-widest">{signal.market} • {signal.timeframe || "1H"}</p>
          </div>
        </div>
        <div className="flex gap-2">
          <Button 
            variant="outline" 
            size="sm"
            onClick={() => refreshMutation.mutate({ symbol: symbolStr })}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="flex flex-col justify-center space-y-4">
              <div>
                ...
                <div className={`inline-flex px-4 py-2 rounded border text-xl font-bold items-center gap-2 ${getDirectionColor()}`}>
            ...
            <div className="md:col-span-2 grid grid-cols-2 gap-4">
              <div className="bg-secondary p-4 rounded-lg border border-border flex flex-col justify-center">
                ...
                <span className="text-2xl font-bold font-mono">{signal.price.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 6 })}</span>
              </div>
              <div className="bg-secondary p-4 rounded-lg border border-border flex flex-col justify-center">
              ...
              <div className="bg-secondary p-4 rounded-lg border border-border flex flex-col justify-center">
                <span className="text-xs text-muted-foreground uppercase tracking-wider mb-1">Volume 24h</span>
                <span className="text-lg font-mono">{signal.volume24h ? signal.volume24h.toLocaleString() : "-"}</span>
              </div>
              <div className="bg-secondary p-4 rounded-lg border border-border flex flex-col justify-center">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Minimal Price Chart (placeholder) */}
        <Card className="bg-card">
          <CardHeader className="border-b border-border">
            <CardTitle className="text-lg flex items-center gap-2">
            ...
            {candles && candles.length > 0 ? (
              <div className="h-64 flex items-end justify-between gap-1 mt-4">
                {candles.slice(-20).map((c, i) => {
                  const min = Math.min(...candles.map(ca => ca.low));
                  const max = Math.max(...candles.map(ca => ca.high));
                  const range = max - min;
                  const height = Math.max(10, ((c.high - c.low) / range) * 100);
                  const isUp = c.close >= c.open;
                  
                  return (
                    <div 
                      key={i} 
                      className={`w-full rounded-sm ${isUp ? 'bg-primary' : 'bg-destructive'}`}
                      style={{ height: `${height}%` }}
                      title={`Open: ${c.open}, Close: ${c.close}`}
                    />
                  );
                })}
              </div>
            ) : (
               <div className="h-64 flex items-center justify-center border border-dashed border-border rounded-lg text-muted-foreground text-sm uppercase tracking-widest">
                 No Data
               </div>
            )}
